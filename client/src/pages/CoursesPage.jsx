...
const empty = { code: '', title: '', description: '', creditUnits: 3, level: 100, department: 'Computer Science' };
...
export default function CoursesPage() {
    ...
    const openEdit = (c) => {
        setForm({ code: c.code, title: c.title, description: c.description || '', creditUnits: c.creditUnits, level: c.level, department: c.department, _id: c.id });
        setModal('edit');
    };
                ...
                courses.length === 0 ? <div className="empty"><BookOpen size={48} /><p>No courses found</p></div> : (
                    <>
                        <div className="table-wrap">
                            <table>
                                <thead><tr><th>Code</th><th>Title</th><th>Department</th><th>Level</th><th>Credits</th><th>Status</th>{isAdmin && <th>Actions</th>}</tr></thead>
                                <tbody>
                                    {courses.map(c => (
                                        <tr key={c.id}>
                                            <td>
                                                <Link to={`/courses/${c.id}`} style={{ color: 'var(--accent)', fontWeight: 700, fontFamily: 'monospace' }}>{c.code}</Link>
                                            </td>
                                            <td style={{ fontWeight: 500 }}>{c.title}</td>
                                            <td style={{ fontSize: '0.85rem' }}>{c.department}</td>
                                            <td><span className="badge badge-blue">{c.level}L</span></td>
                        ...
                        <form onSubmit={handleSave}>
                            <div className="modal-body">
                                <div className="form-row">
                                    <div className="form-group"><label>Course Code *</label><input required placeholder="e.g. CSC201" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} /></div>
                                    <div className="form-group"><label>Credit Units *</label><input type="number" required min={1} max={6} value={form.creditUnits} onChange={e => setForm(f => ({ ...f, creditUnits: Number(e.target.value) }))} /></div>
                                </div>
